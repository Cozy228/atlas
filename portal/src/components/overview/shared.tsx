/**
 * shared pieces for the `/overview`
...
export function HealthDot({ health, className }: { health: ServiceHealth; className?: string }) {
  return (
    <span
      aria-hidden
      className={cn("size-2 shrink-0 rounded-full", HEALTH_DOT[health], className)}
    />
  );
}
...
export function DeployStatusLabel({ status }: { status: DeployStatus }) {
  ...
  return (
    <span
      className={cn("inline-flex items-center gap-1 text-[12px] font-semibold", meta.className)}
    >
  ...
  return (
    <ul className="flex flex-col overflow-hidden rounded-[4px] border border-border bg-card">
      {shown.map((item, i) => (
        <li
          key={item.id}
          className={cn("flex gap-3 px-3.5 py-3", i > 0 && "border-t border-border")}
        >
          <span
            aria-hidden
            className={cn("mt-1.5 size-2 shrink-0 rounded-full", ATTENTION_DOT[item.tone])}
          />
