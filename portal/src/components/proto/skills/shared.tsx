/**
 * PROTOTYPE (production candidate) — shared pieces for the `/proto/skills`
...
export function CommandBlock({ command, label }: { command: string; label?: string }) {
  return (
    <div className="flex flex-col gap-1.5">
      {label ? (
        <span className="w-fit bg-background font-mono text-[10px] font-semibold uppercase tracking-[0.12em] text-muted-foreground">
          {label}
        </span>
      ) : null}
...
export function InstallBay({ skill }: { skill: Skill }) {
  ...
  return (
    ...
    >
      ...
      <div role="radiogroup" aria-label="Install context" className="flex rounded-[5px] bg-muted p-0.5">
        {INSTALL_MODES.map((m) => (
          <button
            key={m.id}
            type="button"
      ...
      {mode === "ci" ? (
        ...
        <CommandBlock command={mode === "add" ? skillInstallCommand(skill.id) : skillRunCommand(skill.id)} />
      )}
