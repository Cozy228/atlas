/**
 * PROTOTYPE (production candidate) — Catalog direction "Adopted mainline".
 *
 * The review-liked mainline `/catalog` design carried into the proto suite:
 ...
 * not modified.
 *
 * Two deliberate proto-suite differences:
 *   - capability details link to `/proto/capability` (the proto datasheet
 *     replaces `/catalog/$topicId` for capabilities);
 *   - no Sources tab — the suite's sources surface is `/proto/sources`.
 */
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div key={tab} className="animate-in fade-in duration-150 motion-reduce:animate-none">
        ...
        {tab === "landing-zones" ? (
          <TopicWorkspace type="landing-zone" topics={landingZones} zone={zone} query={query} view={view} />
        ) : null}
        {tab === "guardrails" ? (
          <TopicWorkspace type="guardrail-area" topics={guardrails} zone={zone} query={query} view={view} />
        ) : null}
      </div>
    </div>
  );
...
function TableView({
  ...
  const open = (topicId: string) => {
    if (type === "guardrail-area") {
      ...
      // The proto suite's capability detail is the fixed exemplar datasheet.
      void navigate({ to: "/proto/capability" });
    } else {
...
function TopicNameLink({ topic, type }: { topic: Topic; type: Topic["topic_type"] }) {
  ...
  if (type === "capability") {
    return (
      <Link to="/proto/capability" onClick={stop} className={className}>
        {topic.name}
      </Link>
    );
  }
  return (
    <Link to="/catalog/$topicId" params={{ topicId: topic.id }} onClick={stop} className={className}>
      {topic.name}
    </Link>
  );
}
...
function CapabilityCard({
    ...
    <>
      ...
      <div className="flex flex-wrap items-center gap-1.5">
        {visibleChips.map((location) => {
          const cell = service!.availability[location.id]!;
          return (
            <RegionChip key={location.id} status={cell.status} label={location.label} note={cell.note} />
          );
        })}
  ...
  // The proto suite's capability detail is the fixed exemplar datasheet.
  return (
    <Link to="/proto/capability" className={CARD_BASE}>
      {body}
    </Link>
  );
}
