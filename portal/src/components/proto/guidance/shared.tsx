/**
 * PROTOTYPE (production candidate) — shared vocabulary for the
 * `/proto/guidance` index + detail directions: type metadata, status chips,
...
import { ActionControl } from "@/components/guidance/shared";
import { Badge } from "@/components/ui/badge";
import type { Guidance, GuidanceStep, GuidanceType } from "@/lib/guidance";
...
export function DestinationFlag({ title, className }: { title: string; className?: string }) {
  return (
    <span className={cn("flex min-w-0 items-center gap-1.5 text-[12px] text-muted-foreground", className)}>
  ...
  return (
    <ul className="flex flex-col divide-y divide-border rounded-[4px] border border-border bg-card">
      {tasks.map((task) => {
        ...
        return (
          <li key={task.id} className="flex flex-wrap items-center justify-between gap-3 px-3.5 py-2.5">
            <button
              type="button"
              role="checkbox"
              aria-checked={done}
  ...
  return (
    <ul className="flex flex-col divide-y divide-border rounded-[4px] border border-border bg-card">
      {sourceIds.map((sourceId) => {
        ...
        return (
          <li key={sourceId} className="flex flex-wrap items-center justify-between gap-2 px-3.5 py-2">
            {source ? (
              <>
                <Link
                  to="/sources/$sourceId"
...
export function stepSupport(guidance: Guidance, step: GuidanceStep): { team: string; channel: string } {
