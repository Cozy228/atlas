 ...
 * from the mainline; the layout is the prototype's own.
 */
 ...
 */
const KEY_SECTIONS: Record<Source["source_class"], ReadonlyArray<{ anchor: string; text: string }>> = {
  "terraform-module": [
    { anchor: "§ inputs", text: "Variables gate the blast radius: scope, tags, and a required owner are mandatory inputs." },
    { anchor: "§ iam", text: "Provisions least-privilege roles; wildcard policies are rejected at plan time." },
    { anchor: "§ outputs", text: "Exports the network and identifier outputs downstream modules depend on." },
  ],
  "confluence-page": [
    { anchor: "# overview", text: "Describes the approved request-and-approval flow, with the required reviewers per environment." },
    { anchor: "# steps", text: "Walks the change from intake to promotion, linking the runbook for the rollback path." },
    { anchor: "# owners", text: "Names the accountable team and the escalation channel for exceptions." },
  ],
  "policy-document": [
    { anchor: "§ controls", text: "States the mandatory controls and the enforcement point each one is checked at." },
    { anchor: "§ exceptions", text: "Defines the exception process, its approvers, and the maximum waiver window." },
    { anchor: "§ mapping", text: "Cites the upstream standard each control maps to, for audit traceability." },
  ],
};
...
function revisionLog(source: Source): ReadonlyArray<{ date: string; label: string }> {
  ...
  const back = (cycles: number) =>
    fmtDate(new Date(reviewedMs - windowMs * cycles).toISOString());
  return [
    { date: fmtDate(source.last_reviewed_at), label: `Reviewed by ${source.steward}; authority reaffirmed.` },
    {
  ...
  return (
    <div className="rounded-[4px] border border-warning/30 bg-warning/10 px-4 py-3">
      ...
      <p className="mt-1 text-[12px] leading-[1.5] text-muted-foreground">
        ...
        <code className="rounded-[2px] bg-card px-1 py-0.5 font-mono text-[11px]">access_denied</code>.
      ...
      {anchors.map((anchor, i) => (
        ...
        >
          <span className="flex min-w-0 flex-col gap-0.5">
            ...
            <code className="font-mono text-[10.5px] text-muted-foreground">{anchor.anchor_id}</code>
          </span>
    ...
    <div className="mx-auto flex w-full max-w-[960px] flex-col gap-7">
      ...
      <div className="grid gap-x-10 gap-y-7 lg:grid-cols-[minmax(0,1fr)_260px]">
          ...
          <section className="flex flex-col gap-2.5">
            ...
            <ul className="flex flex-col gap-2.5">
              {KEY_SECTIONS[source.source_class].map((section) => (
                <li key={section.anchor} className="flex flex-col gap-0.5 border-l-2 border-border pl-3">
                  <code className="font-mono text-[10.5px] text-muted-foreground">{section.anchor}</code>
                  ...
                  <li key={r.id} className={cn(i > 0 && "border-t border-border")}>
                    ...
                    >
                      <span className="flex min-w-0 items-center gap-2">
                        <span aria-hidden className={cn("size-2 shrink-0 rounded-full", FRESHNESS_META[classifyFreshness(r)].dot)} />
            ...
            <ol className="flex flex-col">
              {revisionLog(source).map((rev, i) => (
                ...
                >
                  ...
                  <span className="text-[12.5px] leading-[1.5] text-foreground/90">{rev.label}</span>
                </li>
              ))}
            </ol>
          </section>
        ...
        <aside className="flex min-w-0 flex-col gap-2.5">
          ...
          >
            <span aria-hidden className={cn("mt-1 size-2 shrink-0 rounded-full", FRESHNESS_META[fresh].dot)} />
