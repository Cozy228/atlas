...
export function OverviewDashboard() {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <section aria-label="Condition and attention" className="grid items-start gap-4 lg:grid-cols-[220px_minmax(0,1fr)]">
      ...
      <section aria-label="Promotions and tickets" className="grid items-start gap-5 lg:grid-cols-2">
        <PendingPromotions />
...
function MiniStat({ label, value, warn }: { label: string; value: number; warn?: boolean }) {
  return (
    <div className="flex items-baseline justify-between gap-2">
      ...
      <dd className={cn("font-mono text-[12px] font-semibold tabular-nums text-foreground", warn && "text-warning")}>
        {value}
      </dd>
    </div>
  );
        ...
        {recent.map((run, i) => (
          ...
          >
            <span className="flex min-w-0 flex-col">
              <span className="truncate text-[12.5px] font-semibold text-foreground">{run.service}</span>
    ...
    <section aria-label="Security scans" className="flex flex-col gap-2.5">
      ...
      <div className="flex flex-col gap-3 rounded-[4px] border border-border bg-card p-3.5">
        ...
        <ul className="flex flex-col border-t border-border pt-1">
          {SCANS.map((s, i) => (
            <li key={s.serviceId} className={cn("flex items-center justify-between gap-3 py-1.5", i > 0 && "border-t border-border")}>
...
function ScanFigure({ value, label, tone }: { value: number; label: string; tone?: "critical" | "warning" }) {
  return (
  ...
  return (
    <section aria-label="Open tickets" className="flex flex-col gap-2.5">
      <h2 className="flex items-baseline justify-between gap-2">
        ...
        <span className="bg-background font-mono text-[10.5px] tabular-nums text-muted-foreground">{tickets.length}</span>
      </h2>
      <ul className="overflow-hidden rounded-[4px] border border-border bg-card">
        {tickets.map((t, i) => (
          <li key={t.id} className={cn("flex items-start gap-2.5 px-3.5 py-2", i > 0 && "border-t border-border")}>
            <PriorityChip priority={t.priority} />
            <span className="flex min-w-0 flex-1 flex-col">
              <span className="truncate text-[12.5px] font-semibold text-foreground">{t.title}</span>
              <span className="font-mono text-[10px] text-muted-foreground">
...
function PendingPromotions() {
  ...
  return (
    <section aria-label="Pending promotions" className="flex flex-col gap-2.5">
      ...
      <ul className="overflow-hidden rounded-[4px] border border-border bg-card">
        {shown.map((s, i) => (
          <li key={s.id} className={cn("flex items-center justify-between gap-3 px-3.5 py-2", i > 0 && "border-t border-border")}>
