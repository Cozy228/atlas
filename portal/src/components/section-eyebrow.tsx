...
export function SectionEyebrow({ eyebrow, title, description, className }: SectionEyebrowProps) {
  return (
    <header className={cn("flex flex-col gap-1.5", className)}>
      ...
      {title ? (
        <h2 className="w-fit type-section font-semibold tracking-[-0.03em] text-foreground">
          {title}
        </h2>
      ) : null}
      {description ? (
        <p className="w-fit max-w-[52ch] text-sm leading-6 text-muted-foreground">{description}</p>
      ) : null}
    </header>
  );
}
