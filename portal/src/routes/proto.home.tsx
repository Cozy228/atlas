...
import { DOMAIN_BLURBS } from "@/components/proto/catalog/data";
import { HomeWelcome } from "@/components/proto/home/welcome";
import type { DomainService, HomeLoaderData } from "@/components/proto/home/data";
...
export const Route = createFileRoute("/proto/home")({
  loader: async ({ context }): Promise<HomeLoaderData> => {
    ...
    for (const service of services) {
      ...
      (byDomain.get(service.domain) ?? byDomain.set(service.domain, []).get(service.domain)!).push(entry);
    }
    const domains = [...byDomain.entries()]
      ...
      .map(([domain, entries]) => {
        ...
        return {
          ...
          count: sorted.length,
          preview: sorted.slice(0, 3).map((s) => s.name).join(" · "),
