...
for (let i = 0; i < 40; i++) {
  ...
  if (o.done) {
    res = { elapsed: o.elapsed, longtasks: o.lt, ltc: o.ltc, path: o.path };
    break;
  }
}
...
const fin = JSON.parse(
  await evalJs(`JSON.stringify({lt:Math.round(window.__sw?.lt??0),ltc:window.__sw?.ltc??0})`),
);
console.log(`TAB SWITCH (throttled) /  ->  /availability`);
console.log(
  `  time-to-matrix: ${res.elapsed}ms | main-thread blocking (long-tasks) during switch: ${fin.lt}ms across ${fin.ltc} tasks`,
);
ws.close();
