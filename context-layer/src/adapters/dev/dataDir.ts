 ...
 *      the first ancestor that holds the `data/` registry manifests. This works both when
 *      running from source (`context-layer/src/adapters/dev/…`) and from the bundled
...
export function resolveDataDir(): string {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    // Sentinel: the registry manifest marks the real data/ dir (vs any other `data` folder).
